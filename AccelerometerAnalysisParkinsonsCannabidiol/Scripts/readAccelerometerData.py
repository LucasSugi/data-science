    ...
    for path, dirs, files in os.walk(directory):
            ...
            for f in files:
                    if f.endswith(".txt"):
                            ...
                            xcorr_xy = np.correlate(matrix[:,0],matrix[:,1],mode='same')
                            xcorr_peak_xy = np.max(xcorr_xy)
                            xcorr_lag_xy = (np.argmax(xcorr_xy))/len(xcorr_xy)

                            xcorr_xz = np.correlate(matrix[:,0],matrix[:,2],mode='same')
                            xcorr_peak_xz = np.max(xcorr_xz)
                            xcorr_lag_xz = (np.argmax(xcorr_xz))/len(xcorr_xz)

                            xcorr_yz = np.correlate(matrix[:,1],matrix[:,2],mode='same')
                            ...
                            jerk = np.diff(ts1)
                            jerk=[np.nanmean(jerk),np.nanstd(jerk),skew(jerk[~np.isnan(jerk)]),kurtosis(jerk[~np.isnan(jerk)])]
                            ...
                            data[name][drug][measure] = [ts1, ts2, evalno,RMS,r,mean,var,sk,kurt,jerk]
